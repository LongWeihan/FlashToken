...
## 2) Baseline and compared methods
...
- FlashToken:
  - fixed_prefix: `cached_encoded_tokens` is roughly the tail work after removing the stable prefix part
  - append_only: `cached_encoded_tokens == sum(len(tokens_to_append))`
...
2) append_only: initial text + many appends (models chat history growth)
...
- `append_only_work_series.png`: why baseline grows hotter over time (more tokens per turn)
